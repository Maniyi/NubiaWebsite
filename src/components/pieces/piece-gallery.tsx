      ...
      <Container>
        ...
        <ol className="piece-catalogue__grid">{pieceProfiles.map((piece, index) => (
          <li key={piece.id}><article className="piece-card" data-selected={piece.id === selectedId}>
            <div className="piece-card__portrait"><span aria-hidden="true">{String(index + 1).padStart(2, "0")}</span><AssetImage assetId={piece.assetId} alt={piece.alt} sizes="(min-width: 1200px) 260px, (min-width: 768px) 45vw, calc(100vw - 64px)" loading={piece.id === selectedId ? "eager" : undefined} /></div>
